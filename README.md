...
本プロジェクトは、深層学習モデル（多層パーセプトロン、MLP）を用いて顧客の解約（チャーン）を予測し、解約の意思決定に最も寄与する要因を分析することを目的としています。顧客データを詳細に分析・モデル化することで、高精度な予測モデルを構築するだけでなく、特徴量重要度（Feature Importance）の分析を通じて、顧客離れの核心的な要因を明らかにしました。
